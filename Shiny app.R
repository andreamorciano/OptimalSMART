                ...
                tabItems(
                  tabItem(tabName = "introduction",
                          fluidRow(column(10,offset = 1,
                                          ...
                                          span("Version 0.2.0., created by Andrea Morciano."),
                                          br(),
                                          br(),
                                          span("This Shiny App is designed to help users derive optimal proportions of patients assigned
                                               to treatments in a SMART design and thus obtain the optimal design. A more general version of this app that allows for more than two treatments is available here"),
                                          a(href="https://andreamorciano.shinyapps.io/OptimalSMART/","here "),
                              
                                          br(),
                                          span("Using the prototypical SMART design as shown in the figure below, users are asked to specify the response
                                               rates for the first-stage treatment options and the weights used in the multiple objective optimal design. 
                                          ...
                                          div("The first tab allows calculation of the locally optimal design. This optimal design is compared to 
                                              the balanced design in terms of relative efficiency and also contour plots for relative efficiency are 
                                              shown. Two scenarios are considered: one where sample size is fixed and cost of treatments is not taken 
                                              into account, and another in which cost of treatments is considered and sample size depends on a budget
                                              constraint. In both scenarios, the response rates and the weights for the multiple-objective optimal 
                                              design should be specified. Furthermore, the total sample size should be specified in the first scenario, 
                                              while the cost per treatment and the budget should be specified in the second scenario.", style = "text-align: justify;"),
                                          br(),
                                          h3("Maximin Design: Overview"),
                                          div("The second tab allows calculation of the maximin design, which provides a solution to the problem of 
                                              the unknown response rates for the first-stage treatment options. The maximin design is compared to the 
                                              balanced design in terms of minimum relative efficiency and contour plots for the relative efficiency 
                                              are shown. As for the locally optimal design, a fixed total sample size or a budgetary constraint is 
                                              used.", style = "text-align: justify;"),
                                          br(),
                                         ...
                                         br(),
                                          div(tags$b("A SMART design example:"), tags$i("An example of a prototypical SMART design. Circled “R” denotes randomization at each stage. 
                                                     p1 and (1-p1 ) are, respectively, the probabilities of receiving the two first-stage treatments:
                                                     physical activity (PHY) and nutrition (NUT). For subjects starting with PHY as first-stage 
                                                     treatment, p2 and (1-p2 ) respectively denote the probability of
                                                     receiving the second-stage treatment options NUT and NUT+PHY. For subjects starting with NUT as first-stage
                                                      treatment, p3 and (1-p3 ) are, respectively, the probabilities of receiving the second-stage treatment options 
                                                     PHY and NUT+PHY . γ1 and  γ2 indicate, 
                                                     respectively, response rates for the first-stage treatments PHY and NUT, while (1-γ1 ) and (1-γ2 ) 
                                                     indicate the proportion of non-responders for the first-stage treatments."),style="text-align: justify;"),
                                          br(),
                                          br(),
                                          br(),
                                          br()
                  ...
                  tabItem(tabName = "optimalcost",
                              ...
                              fluidRow(column(width = 4,
                                              ...
                                              div(
                                                ...
                                                numericInput(inputId = "Cp", label = "Cost physical activity:",
                                                             value = 50,
                                                             min = 0, max = 1000, step = 10),
                                                
                                                numericInput(inputId = "Cn", label = "Cost nutrition:",
                                                             value = 300,
                                                             min = 0, max = 1000, step = 10),
                              ...
                              fluidRow(column(width = 4,
                                              box(title = "", width = 12, status = "primary", solidHeader = T,
                                                  div(
                                                    ...
                                                    numericInput(inputId = "Cp1", label = "Cost physical activity:",
                                                                 value = 50,
                                                                 min = 0, max = 1000, step = 10),
                                                    
                                                    numericInput(inputId = "Cn1", label = "Cost nutrition:",
                                                                 value = 300,
                                                                 min = 0, max = 1000, step = 10),
...
server <- function(input, output, session){
 ...
 output$SMART <- renderImage({
  ...
  table_cache <- matrix(nrow = 0, ncol = 15)
  colnames(table_cache) <- c(paste0(intToUtf8(947),c(1:2)), "Cp", "Cn", "Cpn", "p1.opt", "p2.opt", "p3.opt", "N.opt",
  ...
  observeEvent(input$optval, {
    ...
    anal_values$all <- analytic.prop(input$g1, input$g2, input$Cp, input$Cn, input$w13,
  ...
  output$r_eff <- renderUI({
    ...
    paste("Relative efficiency of the balanced design with respect to the optimal design is:", 
          ...
          format(as.integer(anal_values$final$x$data[,11]), big.mark = " "), ".",
  ...
  table_cache1 <- matrix(nrow = 0, ncol = 16)
  colnames(table_cache1) <- c(paste0(intToUtf8(947),c(1:2)," Range"), "Cp", "Cn", "Cpn", "p1.mmd", "p2.mmd", "p3.mmd", 
  ...
  observeEvent(input$mmdopt, {
    ...
    mmd_values$mmm <- maximin_design(gamma1 = input$g11, gamma2 = input$g21, input$Cp1, input$Cn1, input$w131,
  ...
  output$r_eff_mmd <- renderUI({
    ...
    paste("Relative efficiency of the balanced design with respect to the maximin design is:", 
          ...
          format(as.integer(mmd_values$mmd_value$x$data[,11]), big.mark = " "), ".",
          "Note: this refers to the last row in the above table.")
  })
